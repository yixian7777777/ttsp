...
def create_plots(run, force_recreate=False):
    ...
    plot_trace(run, 'vdd', 'vdd', lambda: get_vdd_traces(run), active_cores, yMin=0, force_recreate=force_recreate)
    plot_trace(run, 'power_budget', 'power_budget', lambda: get_power_budget_traces(run), active_cores, yMin=0, force_recreate=force_recreate)
